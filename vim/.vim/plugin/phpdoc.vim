...
" Tested:		VIM 6.1
"
" Version:		$Id: phpdoc.vim 332 2007-01-24 14:17:21Z raszi $
"
...
if g:foo_DefineAutoCommands
	...
	autocmd BufEnter *.prm inoremap { {<Esc>:call ClassHeader()<CR>a<Esc>:call FuncHeader()<CR>a
	autocmd BufEnter *.prm inoremap ; ;<Esc>:call ClassVar()<CR>a
	autocmd BufLeave *.prm iunmap {
	autocmd BufLeave *.prm iunmap ;

	autocmd BufEnter *.inc inoremap { {<Esc>:call ClassHeader()<CR>a<Esc>:call FuncHeader()<CR>a
	autocmd BufEnter *.inc inoremap ; ;<Esc>:call ClassVar()<CR>a
	autocmd BufLeave *.inc iunmap {
	autocmd BufLeave *.inc iunmap ;
...
function GetParameters(string)
	let l:param = ''
	let l:param = matchstr(a:string, '(\s*\S\+.*)')
	let l:param = matchstr(l:param, '[^()]\+')
	let l:param = matchstr(l:param, '\S\+.*')
...
fun! ClassHeader()
	...
	let @z=@z . l:indent . " *\n"
	let @z=@z . l:indent . " * @author      KARASZI Istvan <ikaraszi@adverticum.com>\n"
	let @z=@z . l:indent . " * @copyright   (c) 2006 by KARASZI Istvan\n"
	" prevent RCS/CVS to expand the Id Keyword.
	let @z=@z . l:indent . " * @version     $" . "Id$\n"
	let @z=@z . l:indent . " * @package     xpps\n"
...
fun! FuncHeader()
	...
	if l:line =~ '\(.*\)'
		let l:parameter = matchstr(l:line, '\(.*\)')
		let l:parameter = GetParameters(l:parameter)
	endif
	let @z=@z . l:indent . "/**\n"
	let @z=@z . l:indent . " *\n"
	...
	if l:returntype != ''
		let @z = @z . l:indent . " * @return    " . l:returntype . "\n"
	endif
	if l:parameter != ''
		let @z = @z . l:indent . " * @param     " . l:parameter . "\n"
	endif
...
fun! ClassVar()
	...
	if l:line =~ '^\s*public\s\+\$[A-Za-z]\+'
		let l:index = matchend(l:line, '^\s*var\s\+\$')
		let l:prefix = matchstr(l:line, '[A-Za-z]', l:index)
		let l:access = 'public'
	endif

	if l:line =~ '^\s*protected\s\+\$[A-Za-z]\+'
		let l:index = matchend(l:line, '^\s*var\s\+\$')
		let l:prefix = matchstr(l:line, '[A-Za-z]', l:index)
		let l:access = 'protected'
	endif

	if l:line =~ '^\s*private\s\+\$'
